...
const contactNames = contacts.map(contact => contact.name)
const sortedContacts = contactNames.sort()
const getFirstTen = sortedContacts.slice(0, 10)
console.log(getFirstTen)

//5. the oldest person's name

const oldestContact = contacts.reduce((previousContact, currentContact) => {
    return previousContact.age < currentContact.age ? currentContact : previousContact;
})
console.log(oldestContact.name)
...
const setFirstAndLastNames = (people) => {
    for (const person of people) {
        const nameToSplit = person.name.split(' ')
        person.firstName = nameToSplit[0]
        person.lastName = nameToSplit[1]
    }
}

setFirstAndLastNames(contacts)
...
const getAverageAge = () => {
    let allAges = contacts.map(contact => contact.age)
    let total = 0
